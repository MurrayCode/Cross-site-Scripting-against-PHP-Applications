...
<div id="gekko_admin_main">
  <h3><?php echo TEMPLATES_TEMPLATE; ?>: <?php echo $filtered_app_name; ?></h3>
  <?php if (!isset($_POST['sure'])): ?>
   ...
   <label><input type="checkbox" name="sure" onclick="javascript:document.getElementById('ok_uninstall_button').disabled=!this.checked;" /><?php echo TEMPLATES_CONFIRM_UNINSTALL; ?></label><br/>
   <label><input type="checkbox" name="everything" /><?php echo TEMPLATES_CONFIRM_DELETE_EVERYTHING; ?></label><br/>
   <br/>
 <?php uninstall_button_ok(); uninstall_button_cancel(); // just a shortcut ?>		   
   </form>
   <?php else: ?>
   <?php if ($uninstall_result===true): ?>
   <p><?php echo TEMPLATES_NOTIFIY_UNINSTALLED; ?></p>
   <?php else: ?>
