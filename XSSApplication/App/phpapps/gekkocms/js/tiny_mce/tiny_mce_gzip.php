...
class TinyMCE_Compressor {
	...
	private function getListOfPlugins()
	{
		$forbidden_listing = array('.','..','notes_','__MACOSX','.svn');
		$plugin_cache_txt = $this->settings['cache_dir'].'/misc/tinymce_plugins.txt';
		if (file_exists($plugin_cache_txt))
		{
			$plugin_list = @unserialize(file_get_contents($plugin_cache_txt));
			if ($plugin_list) return $plugin_list;
		}
		$dir = dirname(__FILE__);
		$cache_path = $dir.'/plugins';
		$plugins = array();
		$dir_handle = @opendir($cache_path);
		while ($file = readdir($dir_handle)) 
		{
			if (!in_array($file,$forbidden_listing) && strpos($file,'.')===false)
			{
				$plugins[] = $file;
			}
		}
		closedir($dir_handle);
		file_put_contents($plugin_cache_txt,serialize($plugins));
		return $plugins;
	}
...
//		$plugins = self::getParam("plugins");
	//	$plugins = explode(',', $this->settings["plugins"]);
		...
		$diskCache = true;//self::getParam("diskcache");
//		if ($diskCache) $this->settings["disk_cache"] = ($diskCache === "true");
		$this->settings["disk_cache"] = true;
		...
		$hash = 'tinymce_'.md5(implode('', $allFiles));
